...
- **写入闭环**：禁止一来源一页。先拆基础知识 → Registry（双链 L0）→ Runbook/FAQ/ADR → 正文自洽 → 按 index-log.md 更新 index/log → 跑 lint。用户可用 [`wiki/index.md`](../../../wiki/index.md) 分组名限制本次写入；范围外标 deferred 并汇报（见 [ingest.md](ingest.md) §写入分组）。
