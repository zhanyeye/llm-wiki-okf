...
| 复杂拓扑 / 排查树要画示意 | `json-canvas` | 可选同目录或分组下 `.canvas`；**不**替代概念页，**不**把可操作步骤只写在画布上 |
...
6. **可选可视化 / 视图**：复杂拓扑确需示意 → `json-canvas`；Registry/type/domain/status 聚合 → `obsidian-bases`。Base 不是第二份资产数据。
