...
`index.md` 是目录 TOC，不是概念页。总入口先按 L0 基础知识、L1 资源目录、运维与设计导航；分组内按能力域或资产种类建子目录。
...
`wiki/index.md` **只在增删/改名分组时**改。日常入库只改被写入的那个分组 `index.md`；值班相关再看 `wiki/常见问题/index.md`。
...
嵌套目录同样遵守“有概念页才建 index”：例如 `wiki/资源目录/数据库/index.md` 列数据库资产，`wiki/资源目录/index.md` 链到 `./数据库/`。不要在父 index 复制列出所有子目录资产。

例外：`wiki/常见问题/index.md` 可在尚无正文时存在，用于列「待入库」症状/问法清单（值班入口）。`wiki/基础知识/` 与 `wiki/资源目录/` 的能力域/资产类 `index.md` 可在尚无概念页时存在，只写「待建」——这是分层骨架，不是空壳知识页。
...
- 同目录概念：`./页名.md`。同目录子文件夹：`./资源目录/`（目录链接，不要写成 `index.md`）。跨目录：仓根绝对路径 `/wiki/资源目录/页.md`。
- 不要把 schema、domain 列表、Skill 说明写进 index。
- index 只做导航；资产聚合视图可由 Obsidian Base 生成，不能成为第二份事实源。

### 常见问题/index.md

按症状或常见问法列。已有正文 → 链接；没有 → `* 待入库：<症状或问法>`。入库后把「待入库」换成链接，不要两条并存。
