...
2. 读 [types.md](types.md) 选 type/目录，按该 type 的固定**中文**标题写正文；frontmatter 字段与示例也在 types.md。文件名与 `title` **必须中文**，禁止把英文来源标题当文件名/`title`。不要套 Karpathy 英文 `# Title` 正文头。工单/纪要等 `sources` 用仓内路径，例如 `raw/tickets/磁盘满.md`（公司 wiki 走 ingest-wiki，`sources` 只写 URL）。
