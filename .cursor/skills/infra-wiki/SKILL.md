...
查询只走 [query.md](query.md)，**不要**先通读 `AGENTS.md`。选目录、写 frontmatter 时再读 [types.md](types.md)；字段细则在 `AGENTS.md` 的 Frontmatter 节。
...
| 故障关闭、复盘、值班结束 | [ingest.md](ingest.md) |
| 把旧文档迁进 wiki | [ingest.md](ingest.md) |
...
- `index.md` / `log.md` 是保留名，不加 `type`。仅 `wiki/index.md` 可有 `okf_version: "0.2"`。
- 链接：`[MinIO](/系统与架构/minio.md)`（相对 wiki 根）。
- 写入后更新所在目录 `index.md`（新分组才改根 index），并在 `wiki/log.md` 顶部追加当日条目。
