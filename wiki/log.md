...
* **Update**: L0 目录改为 [基础知识](/wiki/基础知识/)，按能力域建 OS镜像 / 镜像制作 / 构建资源管理 / 网络管理 / 应用服务 / 资源调度；L1 [资源注册表](/wiki/资源注册表/) 按集群 / 数据库 / 存储 / 中间件 / 可观测分子目录。禁止来源平铺；上层用 Obsidian 双链引用下层。

## 2026-09-02

* **Update**: 知识模型改为 L0 基础知识、L1 资源注册表、L2 运行知识，并加入内容级关系与来源覆盖清单。
