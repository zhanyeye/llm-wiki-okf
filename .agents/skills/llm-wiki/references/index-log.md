...
`index.md` 是目录 TOC，不是概念页。总入口先按 L0 基础知识、L1 资源注册表、运维与设计导航；分组内按能力域或资产种类建子目录。
...
例外：`wiki/故障排查/index.md` 可在尚无正文时存在，用于列「待入库」症状清单（值班入口）。`wiki/基础知识/` 与 `wiki/资源注册表/` 的能力域/资产类 `index.md` 可在尚无概念页时存在，只写「待建」——这是分层骨架，不是空壳知识页。
